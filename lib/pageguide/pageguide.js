        ...
        prototype: {
            ...
            init: function() {
               ...
               // toggle.append('page guide').append('<div><span class="pageguide-tourtitle"></span></div>').append('<a class="pageguide-close" title="Close Guide">close guide &raquo;</a>');
                message.append('<a class="pageguide-close" title="Close Guide"><i class="fa fa-times" aria-hidden="true"></i></a>').append('<span class="pageguide-index"></span>').append('<div class="pageguide-content"></div>').append('<a class="pageguide-back" title="Previous">Previous</a>').append('<a class="pageguide-fwd" title="Next">Next</a>');
            ...
            resetMarker: function ()
            {
                ...
                if ($('.pageguide-toggle-open').hasClass('hoversecondary') || $('.pageguide-toggle-close').hasClass('hoversecondary'))
                {
                    $('.pageguide-toggle-open').removeClass('hoversecondary').addClass('secondarycolor');
                    $('.pageguide-toggle-close').removeClass('hoversecondary').addClass('secondarycolor');
                }
                else
                {
                    $('.pageguide-toggle-open').removeClass('secondarycolor').addClass('hoversecondary');
                    $('.pageguide-toggle-close').removeClass('secondarycolor').addClass('hoversecondary');
                }
            },
            open: function() {
            ...
            _showShadow: function(elem, pulse) {
                ...
                if (!!pulse) $pulse.hide();
                var leftOfset = $t.offset().left - padding / 2;
                var shadowWidth = $t.outerWidth(false) + padding;
                if ($(window).width() < (leftOfset + shadowWidth))
                {
                    shadowWidth = $(window).width() - leftOfset;
                }
                this.$shadow.css({
                    height: $t.outerHeight() + padding,
                    width: shadowWidth, 
                    padding: padding,
                    top: $t.offset().top - padding/2,
                    left: leftOfset,
                    zIndex: zIndex
